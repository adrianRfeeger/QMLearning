...
![Bell-pair diagram showing Alice and Bob sharing one entangled state with correlated measurement outcomes.](assets/entanglement_bell_pair.svg)
...
### Entangled States
If no such decomposition exists, the state is **entangled**. In an entangled state, the qubits share a collective identity: measuring one qubit fixes the correlated outcome for the other in the quantum description, regardless of how far apart they are. This correlation cannot be used to send information faster than light.
...
Quantum mechanics violates these inequalities. Experimental tests (like the CHSH experiment) have consistently ruled out local hidden-variable explanations of the observed correlations.
...
Entanglement provides a way to build communication protocols whose security can be checked using quantum correlations. In **Quantum Key Distribution (QKD)**, such as the Ekert91 protocol:
